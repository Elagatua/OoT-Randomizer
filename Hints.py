...
def write_hint_shop_hints(spoiler: Spoiler, world: World, messages: list[Message]) -> None:
    ...
    for id, hint_text in spoiler.shop_hints[world.id].items():
        update_message_by_id(messages, id, str(hint_text), lang, 0x23)
...
def get_woth_count_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    step_text = lang.hint_text["step"] if item_count == 1 else lang.hint_text["steps"]

    return (GossipText(
        lang.format_from_id(
            "hint_text.woth_count",
            {
                "item_count": item_count,
                "step_text": step_text
            }),
        lang, ['Light Blue'], prefix=lang.hint_text["gossip_prefix"]), None)
...
def get_echo_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    return (GossipText(hint_tuple[0].text.replace(lang.hint_text["gossip_prefix"], ""),
               lang, colors, list(hint_tuple[0].hinted_locations), list(hint_tuple[0].hinted_items),
               lang.hint_text["echo_prefix"]), None)

def get_goal_legacy_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]], custom_prefix: Optional[str] = None) -> HintReturn:

    lang = world.language
    if custom_prefix is None:
        custom_prefix = lang.hint_text["gossip_prefix"]
    ...
    location_text = HintArea.at(location).text(lang, world.settings.clearer_hints, world=None if location.world.id == world.id else location.world.id + 1)

    goal_text_prefix = lang.hint_text["goal_legacy_own"] if location.world.id == world.id else lang.hint_text["goal_legacy_other"]
    ...
    return (GossipText(
        lang.format_from_id(
            "hint_text.goal_legacy",
            {
                "location_text": location_text,
                "goal_text": goal_text
            }),
        lang, ['Light Blue', goal.color], [location.name], [location.item.name], custom_prefix), locations)
...
def get_goal_count_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    goal_prefix = lang.hint_text["goal_count_own"] if len(spoiler.worlds) == 1 else lang.hint_text["goal_count_other"]

    return (GossipText(
        lang.format_from_id(
            "hint_text.goal_count",
            {
        ...
        lang, [goal.color, 'Light Blue'], prefix=lang.hint_text["gossip_prefix"]), None)
...
def get_area_woth_count_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    return GossipText(
        lang.format_from_id(
            "hint_text.escape_from_kak_count",
            {
                "location_text": location_text,
                "item_count": item_count,
                "step_text": step_text
            }),
        lang, ['Red', 'Light Blue'], [], [], prefix=lang.hint_text["gossip_prefix"]), []
...
def get_playthrough_location_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    lang = world.language
    hint_area = HintArea.at(location)
    location_text = hint_area.text(lang, world.settings.clearer_hints, world=hinted_world.id + 1)

    return (GossipText(
        lang.format_from_id(
            "hint_text.way_of_wanderer",
            {
                "location_text": location_text
            }),
        lang, ['Light Blue', 'Yellow'], [location.name], [location.item.name],
        prefix=lang.hint_text["gossip_prefix"]), [location])
...
def get_unlock_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]], hint_type: str) -> HintReturn:
    ...
    lang = world.language
    item_text = get_hint(get_item_generic_name(location.item), lang, hinted_world.settings.clearer_hints).text
    required_item_text = get_hint(get_item_generic_name(required_location.item), lang, hinted_world.settings.clearer_hints).text

    if location.item.world.id == world.id:
        item_player_text = ''
    else:
        item_player_text = lang.format_from_id("hint_text.player_text", {"world": location.item.world.id + 1}) + ' '

    if required_location.item.world.id == world.id:
        required_item_player_text = ''
    else:
        required_item_player_text = lang.format_from_id("hint_text.player_text", {"world": required_location.item.world.id + 1}) + ' '

    if hint_type == 'unlock-playthrough':
        gossip_text_id = "hint_text.unlock_wanderer"
        gossip_colors = ['Light Blue', 'Yellow', 'Light Blue']
    else:
        gossip_text_id = "hint_text.unlock"
    ...
    return (GossipText(
        lang.format_from_id(
            gossip_text_id,
            {
                "required_item_player": required_item_player_text,
                "required_item": required_item_text,
                "item_player": item_player_text,
                "item": item_text
            }),
        lang, gossip_colors, [required_location.name, location.name],
        [required_location.item.name, location.item.name],
        prefix=lang.hint_text["gossip_prefix"]), [required_location, location])
...
def get_barren_hint(spoiler: Spoiler, world: World, checked: dict[CheckedKey, set[CheckedKind]]) -> HintReturn:
    ...
    return GossipText(
        lang.format_from_id(
            "hint_text.foolish",
            {
                "area": area.text(lang, world.settings.clearer_hints, world=None if hinted_world.id == world.id else hinted_world.id + 1)
            }),
    ...
    if hint_dist['always'][1] > 0:
        ...
        for hint in always_locations:
            ...
            if location.name in hinted_world.hint_text_overrides:
                ...
                location_text = get_hint(location.name, world.language, hinted_world.settings.clearer_hints).text
            if '#' not in location_text:
                location_text = '#%s#' % location_text
            item_text = get_hint(get_item_generic_name(location.item), world.language, hinted_world.settings.clearer_hints).text
...
def get_hint_shop_hint(item_name: str, upgrade_level: int, hinted_locations: set[Location], spoiler: Spoiler, world: World, worlds: list[World]) -> GossipText:
    ...
    if effective_level <= 0:
        return GossipText(
            lang.format_from_id(
                "hint_text.shop_obtained",
                {
                    "item_text": item_text
                }),
            lang, ['Green'], [], [], prefix=lang.hint_text["gossip_prefix"])
    ...
    if (len(path_items) >= effective_level):
        item_importance_text = lang.hint_text["shop_importance_path"]
        item_importance_color = 'Green'
        hinted_location = playthrough_items[effective_level - 1]
    elif (len(playthrough_items) >= effective_level):
        item_importance_text = lang.hint_text["shop_importance_wanderer"]
        item_importance_color = 'Yellow'
        hinted_location = playthrough_items[effective_level - 1]
    else:
        item_importance_text = lang.hint_text["shop_importance_neglected"]
        item_importance_color = 'Pink'
        hinted_location = random.choice(foolish_world_items)

    hint_area = HintArea.at(hinted_location)
    location_text = hint_area.text(lang, world.settings.clearer_hints, world=None if hinted_location.world.id == world.id else hinted_location.world.id + 1)
    hinted_locations.add(hinted_location)

    return GossipText(
        lang.format_from_id(
            "hint_text.shop_hoards",
            {
                "location_text": location_text,
                "importance": item_importance_text,
                "item_text": item_text
            }),
        lang, ['Light Blue', item_importance_color], [hinted_location.name],
        [hinted_location.item.name], prefix=lang.hint_text["gossip_prefix"])
